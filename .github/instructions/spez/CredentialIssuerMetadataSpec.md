...
* Because the Swiss Profile relies on Signed Metadata, the endpoint MUST additionally support the `application/jwt` media type to deliver the signed Credential Issuer Metadata. [Document: OpenID for Verifiable Credential Issuance 1.0, Chapter: 12.2.2]
...
* Since the Swiss Profile requires cryptographic proofs, the response schema MUST define the `nonce_endpoint` property as REQUIRED (Type String/URL). [Document: OpenID for Verifiable Credential Issuance 1.0, Chapter: 11.2.3]
* The `notification_endpoint` property MUST NOT be present, as its usage is explicitly forbidden for privacy reasons. [Document: Swiss Profile Issuance 1.0, Chapter: 12.2]
...
* The response schema MUST define the `credential_request_encryption` object advertising the supported request-encryption parameters. [Document: OpenID for Verifiable Credential Issuance 1.0, Chapter: 11.2.3]
* The response schema MUST define the `credential_response_encryption` object advertising the supported response-encryption parameters (`alg_values_supported`, `enc_values_supported`, `zip_values_supported`, `encryption_required`). [Document: OpenID for Verifiable Credential Issuance 1.0, Chapter: 11.2.3]
* If the `batch_credential_issuance` object is present, its `batch_size` property MUST be an integer greater than or equal to 10. [Document: Swiss Profile Issuance, Chapter: 12.2.4]
* When Signed Metadata is served (`application/jwt`), the `kid` JOSE header claim is REQUIRED and MUST be an absolute fragment containing a DID as described in swiss-profile-anchor. [Document: Swiss Profile Issuance 1.0, Chapter: 12.2.2]
* When Signed Metadata is served, the payload MUST declare `"profile_version": "swiss-profile-issuance:1.0.0"`. [Document: Swiss Profile Issuance 1.0, Chapter: 12.2.2]
* According to the Trust Protocol, the schema MUST define the `credential_issuer_identity_trust_statement` property at the top level as REQUIRED (Type String/JWT) to prove the issuer's identity. [Document: Trust Protocol 2.0, Chapter: Trust Markers]
...
* Within each credential configuration in `credential_configurations_supported`, the `cryptographic_binding_methods_supported` property MUST be defined. [Document: OpenID for Verifiable Credential Issuance 1.0, Chapter: 11.2.3]
* Within each credential configuration in `credential_configurations_supported`, the `proof_types_supported` property MUST be defined. [Document: OpenID for Verifiable Credential Issuance 1.0, Chapter: 11.2.3]
* Within each proof type in `proof_types_supported`, the `key_attestations_required` property MUST be defined. [Document: OpenID for Verifiable Credential Issuance 1.0, Chapter: 11.2.3]
