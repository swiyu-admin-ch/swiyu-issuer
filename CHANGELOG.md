...
- New public service methods to support renewal
    ...
    - [Possible breaking change] The change introduces a new state machine with different states for offers and the management of offers
    - New env variables: 
      - `RENEWAL_FLOW_ENABLED` (default: false) to enable the renewal functionality
      - `RENWEAL_API_ENDPOINT`: (no default) to set the renewal endpoint where the offer data can be fetched from
