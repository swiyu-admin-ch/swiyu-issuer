...
- Improve health-check behavior to avoid false negatives and make the service more resilient when external
  status-registry calls fail (#759).
- Allow deferred credential requests to be submitted without subject data so issuers accept requests that omit subject
  attributes (#769).
