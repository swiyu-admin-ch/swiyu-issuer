...
public class SignerConfig {
    ...
    public JWSSigner defaultSigner() throws Exception {
        return switch (sdjwtProperties.getKeyManagementMethod()) {
            case "key" ->
                // We are currently only supporting EC Keys
                    fromEC(ECKey.parseFromPEMEncodedObjects(sdjwtProperties.getPrivateKey()).toECKey());

            case "pkcs11" -> {
                Provider provider = Security.getProvider("SunPKCS11").configure(sdjwtProperties.getHsm().getPkcs11Config());
                Security.addProvider(provider);
                var hsmKeyStore = KeyStore.getInstance("PKCS11", provider);
                hsmKeyStore.load(null, sdjwtProperties.getHsm().getUserPin().toCharArray());
                var privateKey = ECKey.load(hsmKeyStore, sdjwtProperties.getHsm().getKeyId(), sdjwtProperties.getHsm().getUserPin().toCharArray());
            ...
            case "securosys" -> {
                ...
                (new PrintStream(baos)).println(
                        sdjwtProperties.getHsm().getSecurosysStringConfig()
                );
                ...
                var privateKey = (ECPrivateKey) hsmKeyStore.getKey(sdjwtProperties.getHsm().getKeyId(), sdjwtProperties.getHsm().getUserPin().toCharArray());
                yield fromEC(privateKey, provider);
            }
            default ->
                    throw new IllegalArgumentException(String.format("Key management method \"%s\" not supported", sdjwtProperties.getKeyManagementMethod()));
