package ch.admin.bit.swiyu.issuer.management.service;

import ch.admin.bit.swiyu.issuer.management.api.dto.StatusListCreateDto;
import ch.admin.bit.swiyu.issuer.management.config.ApplicationProperties;
import ch.admin.bit.swiyu.issuer.management.config.StatusListProperties;
import ch.admin.bit.swiyu.issuer.management.domain.credential_offer_status.CredentialOfferStatusEntity;
import ch.admin.bit.swiyu.issuer.management.domain.ecosystem.StatusRegistryClient;
import ch.admin.bit.swiyu.issuer.management.domain.status_list.StatusListEntity;
import ch.admin.bit.swiyu.issuer.management.domain.status_list.StatusListRepository;
import ch.admin.bit.swiyu.issuer.management.domain.status_list.TokenStatsListBit;
import ch.admin.bit.swiyu.issuer.management.domain.status_list.TokenStatusListToken;
import ch.admin.bit.swiyu.issuer.management.exception.BadRequestException;
import ch.admin.bit.swiyu.issuer.management.exception.ConfigurationException;
...
public class StatusListService {
    ...
    @Transactional
    public void revoke(Set<CredentialOfferStatusEntity> offerStatusSet) {
        Set<CredentialOfferStatusEntity> revokableStatusSet = offerStatusSet.stream().filter(credentialOfferStatus -> canRevoke(credentialOfferStatus.getStatusList())).collect(Collectors.toSet());
        if (revokableStatusSet.isEmpty()) {
            throw new BadRequestException("No Status List which supports revocation found");
        }
        for (CredentialOfferStatusEntity offerStatus : revokableStatusSet) {
    ...
    @Transactional
    public void suspend(Set<CredentialOfferStatusEntity> offerStatusSet) {
        Set<CredentialOfferStatusEntity> suspendableStatusSet = offerStatusSet.stream().filter(credentialOfferStatus -> canSuspend(credentialOfferStatus.getStatusList())).collect(Collectors.toSet());
        if (suspendableStatusSet.isEmpty()) {
            throw new BadRequestException("No Status List which supports suspension found");
        }
        for (CredentialOfferStatusEntity offerStatus : suspendableStatusSet) {
            switch (offerStatus.getStatusList().getType()) {
    ...
    @Transactional
    public void revalidate(Set<CredentialOfferStatusEntity> offerStatusSet) {
        Set<CredentialOfferStatusEntity> suspendableStatusSet = offerStatusSet.stream().filter(credentialOfferStatus -> canSuspend(credentialOfferStatus.getStatusList())).collect(Collectors.toSet());
        if (suspendableStatusSet.isEmpty()) {
            throw new BadRequestException("No Status List which supports suspension found");
        }
    ...
    @Transactional
    public List<StatusListEntity> findByUriIn(List<String> statusListUris) {
     ...
     */
    private void updateTokenStatusList(CredentialOfferStatusEntity offerStatus, int statusValue) {
        StatusListEntity statusList = offerStatus.getStatusList();
        try {
    ...
    private StatusListEntity initTokenStatusListToken(StatusListCreateDto statusListCreateDto) {
    ...
    private SignedJWT buildStatusListJWT(StatusListEntity statusListEntity, TokenStatusListToken token) {
