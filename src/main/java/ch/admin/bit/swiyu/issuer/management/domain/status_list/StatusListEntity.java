...
public class StatusListEntity {
    ...
    public UUID getRegistryId() {
        /**
         * Currently we have a public interface which inputs only the whole URL for the statuslist.
        ...
        if (matcher.find()) {
            ...
            log.warn(String.format("Extracting datastore entry from the status list uri %s using regex %s failed", uri, uuidPattern));
