...
/**
 * Interceptor for status registry requests. Injects the authorization token from the
 * token provider.
 */
@Component
...
public class StatusRegistryTokenInterceptor implements ClientHttpRequestInterceptor {
    ...
    public ClientHttpResponse intercept(HttpRequest request, byte[] body, ClientHttpRequestExecution execution) throws IOException {
        ...
        if(response.getStatusCode() == HttpStatus.UNAUTHORIZED) {
            request.getHeaders().add("Authorization", "Bearer " + statusRegistryTokenDomainService.forceRefreshAccessToken());
            response = execution.execute(request, body);
        }
        return response;
    }
