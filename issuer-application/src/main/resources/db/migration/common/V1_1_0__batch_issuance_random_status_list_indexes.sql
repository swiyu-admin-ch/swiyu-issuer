...
or replace view available_status_list_indexes as (
    with status_list_indexes as (
        select id, uri, generate_series(0, max_length) index from status_list),
    used_indexes as (
        select status_list_id, index from credential_offer_status
    ),
    free_indexes as (
       select i.index as free_index, i.uri as status_list_uri from status_list_indexes i where i.index NOT IN (select index from used_indexes)
    )
