...
public class StatusListPersistenceService {
    ...
    private List<StatusList> updateTokenStatusList(Set<CredentialOfferStatus> offerStatus, TokenStatusListBit bit) {
        ...
        List<StatusList> updatedEntries = new ArrayList<>(statusListIds.size());
        List<StatusListRegistryUpdate> statusListRegistryUpdateEvents = new ArrayList<>();
        ...
        for (Map.Entry<UUID, List<Integer>> entry : statusListIds.entrySet()) {
            ...
            try {
                ...
                updatedEntries.add(statusList);

                // Prepare registry update if enabled
                if (!applicationProperties.isAutomaticStatusListSynchronizationDisabled()) {
                    statusListRegistryUpdateEvents.add(new StatusListRegistryUpdate(statusList, token));
                }
            } catch (IOException e) {
        ...
        statusListRepository.saveAll(updatedEntries);

        // Publish events for registry updates (will fire AFTER commit)
        for (StatusListRegistryUpdate update : statusListRegistryUpdateEvents) {
    ...
    public void publishToRegistry(StatusListRegistryUpdate update) {
        try {
            ...
            log.error("Failed to publish status list {} to registry.", update.statusList().getId());
        }
    }
